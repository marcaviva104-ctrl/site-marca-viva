...
function renderProduct() {
    ...
    // Preço
    if (isLoggedIn) {
        document.getElementById('product-price').innerText = `R$ ${currentProduct.price.toFixed(2).replace('.', ',')}`;
        document.getElementById('price-unit-label').style.display = 'block';
    } else {
...
function addToCart() {
    ...
    window.cartService.addToCart(currentProduct, qty, customization);

    // Sucesso
    Swal.fire({
        icon: 'success',
        title: 'Adicionado ao Carrinho!',
        text: `${qty}x ${currentProduct.name}`,
