...
const OrderManager = {
    ...
    /** Pedidos (protocols) apenas entre as datas — evita carregar todo o historico no financeiro. */
    ...
    /**
     * Mesmo recorte de datas que getOrdersBetween, sem join em protocol_items — uso no Financeiro (rápido, não estoura timeout).
     */
    async getOrdersBetweenForFinancial(startDate, endDate) {
        if (!window.supabase) return [];
        const s = startDate instanceof Date ? startDate : new Date(startDate);
        const e = endDate instanceof Date ? endDate : new Date(endDate);
        ...
        const FIN_COLS =
            'id, client_id, client_name, client_email, client_phone, created_at, total_amount, status, payment_status';
        const { data, error } = await window.supabase
            .from('protocols')
            .select(FIN_COLS)
            .gte('created_at', s.toISOString())
        ...
        if (error) {
            console.error('Error fetching orders for financial (light):', error);
            return [];
        }
        return this._mapProtocolsToOrders(data || []);
    },
